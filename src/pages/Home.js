...
import { faUserCircle, faShoppingCart, faStar } from '@fortawesome/free-solid-svg-icons'
...
class Home extends Component {
    ...
    render() {
        const storedName = (localStorage.getItem('user_name') || localStorage.getItem('user_email') || '').trim();
        const fallbackName = storedName.includes('@') ? storedName.split('@')[0] : storedName;
        const displayName = fallbackName || 'Customer';
                                ...
                                <div className="a_mainbox1 p-3">
                                    ...
                                    <div className="a_mainbox1_inner w-100 h-100">
                                        <div className="row m-0" style={{ height: '65px' }}>
                                            ...
                                            <div className="col-8">
                                                <h4 className="m-0">Hi, <span className="custName">{displayName}</span></h4>
    ...
    renderProductGrid() {
        ...
        const categories = ['Electronics', 'Fashion', 'Books', 'Home & Kitchen', 'Sports', 'Toys'];
        return (
            <div className="container-fluid px-4 py-4 bg-white mt-2 border-top">
                <h3 className="fw-bold mb-1">Shop by Category</h3>
                <p className="text-muted mb-4">20+ products across 6 categories</p>
                    ...
                    return (
                        <div key={category} className="mb-5">
                            <div className="d-flex justify-content-between align-items-center mb-3">
                                <h5 className="fw-bold m-0 border-start border-warning border-4 ps-2">{category}</h5>
                                <a href="/search" className="btn btn-sm btn-outline-secondary">See all</a>
                            </div>
                            <div className="row g-3">
                                {items.map(product => (
                                    <div key={product.id} className="col-6 col-md-3 col-lg-2">
                                        <div className="card h-100 border-0 shadow-sm">
                                            <div className="text-center p-2" style={{ height: 150, overflow: 'hidden', background: '#f8f9fa' }}>
                                                <img src={product.image} alt={product.name} style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                                            </div>
                                            <div className="card-body p-2">
                                                <p className="mb-1 fw-semibold" style={{ fontSize: 13, lineHeight: 1.3 }}>
                                                    {product.name.length > 45 ? product.name.slice(0, 45) + '...' : product.name}
                                                </p>
                                                <span className="badge bg-light text-dark border mb-1" style={{ fontSize: 10 }}>{product.type}</span>
                                                <div className="d-flex align-items-center gap-1 mb-1">
                                                    <FontAwesomeIcon icon={faStar} size="xs" className="text-warning" />
                                                    <small className="text-muted">{product.rating} ({product.reviews.toLocaleString()})</small>
                                                </div>
                                                <div>
                                                    <span className="fw-bold text-danger" style={{ fontSize: 14 }}>Rs.{product.price.toLocaleString()}</span>
                                                    <small className="text-muted text-decoration-line-through ms-1">Rs.{product.originalPrice.toLocaleString()}</small>
                                                </div>
                                                <div className="text-success small fw-bold">{Math.round((1 - product.price / product.originalPrice) * 100)}% off</div>
                                            </div>
                                            <div className="card-footer bg-transparent border-0 p-2">
                                                <div className="d-grid gap-1">
                                                    <a href={'/detail?id=' + product.id} className="btn btn-warning btn-sm fw-semibold" style={{ fontSize: 12 }}>View Details</a>
                                                    <button className="btn btn-outline-warning btn-sm" style={{ fontSize: 12 }} onClick={() => addToCart(product, 1)}>
                                                        <FontAwesomeIcon icon={faShoppingCart} size="xs" className="me-1" />Add to Cart
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
