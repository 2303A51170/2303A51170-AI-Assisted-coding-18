...
class Header extends Component {
    ...
    handleSearchInput = (e) => {
        ...
        if (!keyword) {
            this.setState({ searchTerm, searchSuggestions: [], showSearchSuggestions: false });
            return;
        }
        ...
        const uniqueSuggestions = [...new Set(byName)];

        this.setState({
            searchTerm,
            searchSuggestions: uniqueSuggestions,
            showSearchSuggestions: uniqueSuggestions.length > 0
        });
    }
    handleSearchFocus = () => {
        this.setState((prev) => ({
            z: 'formboder',
            showSearchSuggestions: prev.searchSuggestions.length > 0
        }));
    }
    handleSearchBlur = () => {
        this.setState({ z: '' });
    ...
    handleSuggestionClick = (suggestion) => {
        ...
        this.setState({ searchTerm: q, showSearchSuggestions: false });
        this.props.history.push(`/search?q=${encodeURIComponent(q)}`);
    }
    render() {
        const { t } = this.props;
        const token = localStorage.getItem('token');
        const rawUserName = (localStorage.getItem('user_name') || localStorage.getItem('user_email') || '').trim();
        const fallbackUserName = rawUserName.includes('@') ? rawUserName.split('@')[0] : rawUserName;
        const displayUserName = fallbackUserName || 'Customer';
        return (
            <React.Fragment>
                <style>
                    {myCustomFont}
                ...
                <div className={'AllNavMenu h-100 position-fixed top-0 ' + this.state.AllNavMenu}>
                    <div className="AllNavMenuHeader text-white mhcolor p-3">
                        <h5 className="modal-title " id="staticBackdropLabel"><FontAwesomeIcon icon={faUser} /> Hello, {token ? displayUserName : 'Sign in'}</h5>
                    </div>
                ...
                <header>
                    <div className={'a_header_top p-1 w-100 ' + this.state.y}>
                        ...
                        <form onSubmit={this.handleSearchSubmit} className={'bg-white hform  w-50 d-inline-block ' + this.state.z}>
                                ...
                                <div className="col-10 p-0">
                                    <div className="position-relative h-100">
                                        ...
                                        {this.state.showSearchSuggestions && (
                                            <div className="searchSuggestBox list-group position-absolute w-100">
                                                {this.state.searchSuggestions.map((s) => (
                                                    <button
                                                        key={s}
                                                        type="button"
                        ...
                        <button onMouseOver={this.overylay} onMouseOut={this.hideOverlay} className="a_ht_signin_model_btn position-relative text-start fs-6 text-white btn h-100" style={{ 'width': '8%' }}>
                            <div>Hello, {token ? displayUserName : 'Sign in'}</div>
